...
#include "hw.h"
#include "lmic.h"
#if defined(BRD_LoRa_E5_radio)
#include <stm32wlxx_hal_subghz.h>               // Interface to radio module

SUBGHZ_HandleTypeDef rhandle ;                  // Handle to access SUBGHZ module
HAL_StatusTypeDef    rstat ;                    // Result SUBGHZ functions

uint16_t   lastIrqStatus = 0 ;                  // Last seen IrqStatus

// Overruled Call-back function used by HAL_SUBGHZ_Init(&handle);
void HAL_SUBGHZ_MspInit ( SUBGHZ_HandleTypeDef* hsubghz )
{
    __HAL_RCC_SUBGHZSPI_CLK_ENABLE() ;          // enable clock to sub-ghz module
...
{
    state.sleeping = 0 ;
    rstat = HAL_SUBGHZ_ExecSetCmd ( &rhandle, cmd, (uint8_t*)data, len ) ;
    ASSERT ( rstat == HAL_OK ) ;
}
...
{
    state.sleeping = 0;
    rstat = HAL_SUBGHZ_WriteRegisters ( &rhandle, addr, (uint8_t*)data, len ) ;
    ASSERT ( rstat == HAL_OK ) ;
}
...
{
    state.sleeping = 0;
    rstat = HAL_SUBGHZ_WriteBuffer ( &rhandle, off, (uint8_t*)data, len ) ;
    ASSERT ( rstat == HAL_OK ) ;
}

static void readcmd ( uint8_t cmd, uint8_t* data, uint8_t len )
{
    state.sleeping = 0 ;
    rstat = HAL_SUBGHZ_ExecGetCmd ( &rhandle, cmd, data, len ) ;
    ASSERT ( rstat == HAL_OK ) ;
}
...
{
    state.sleeping = 0;
    rstat = HAL_SUBGHZ_ReadRegisters ( &rhandle, addr, data, len ) ;
    ASSERT ( rstat == HAL_OK ) ;
}
...
{
    state.sleeping = 0 ;
    rstat = HAL_SUBGHZ_ReadBuffer ( &rhandle, off, data, len ) ;
    ASSERT ( rstat == HAL_OK ) ;
}
...
static void Radio_SMPS_Set ( uint8_t level )
{
    ...
    modReg = ReadReg ( SUBGHZ_SMPSC2R ) ;
    modReg &= ( ~SMPS_DRV_MASK ) ;
    WriteReg ( SUBGHZ_SMPSC2R, modReg | level ) ;
}
...
static void SetRfSwitchCtrl ( uint8_t enable )
{
    writecmd ( RADIO_SET_RFSWITCHMODE, &enable, 1 ) ;
}

static void SetTcxoMode()
{
    hal_pin_tcxo ( 1 ) ;                                        // Control radio power
    uint32_t timeout = TCXO_STARTUP_TIME ;
    uint8_t  voltage = TCXO_VOLTAGE1_7V ;
    uint8_t  data[]  = { voltage, ( timeout >> 16)  & 0xff,
                         ( timeout >> 8 ) & 0xff,
                         timeout & 0xff } ;
    writecmd ( RADIO_SET_TCXOMODE, data, sizeof(data));
}
...
{
    uint8_t param[6];
    param[0] = 0x00 ;               // 8 symbols preamble MSB
    param[1] = 0x08 ;               // 8 bytes preamble LSB
    param[2] = getIh(rps) ;         // Header type: implicit header
    param[3] = len ;                // Payload length
    param[4] = !getNocrc(rps) ;     // CRC mode
    param[5] = inv ;                // IQ inversion
...
// GetIrqStatus(), return irq register in 10 bits
// Bit      Meaning
// ---      ------------------------------
//  0       TXDONE
//  1       RXDONE
...
//  4       HeaderValid
//  5       HeaderErr
//  6       CRC error (for LoRa)
//  7       Channel activity detection finished
//  8       CadDetected
//  9       Time-out
uint16_t GetIrqStatus (void)
{
    ...
    readcmd ( CMD_GETIRQSTATUS, buf, 2 ) ;              // Get status bits
    lastIrqStatus = ( buf[0] << 8) | buf[1] ;           // Convert to 16 bits value
    return  lastIrqStatus ;                             // Return the value
}


uint16_t GetLastIrqStatus (void)
{
    return  lastIrqStatus ;                             // Return the last read IRQ status
}
...
{
    // There are 4 masks.  Mask 0 and 1 are used
...
static void CommonSetup (void)
{
    ...
    SetRfSwitchCtrl ( 0 ) ;                     // Not controlled by DIO
    SetTcxoMode() ;
...
static void txlora (void)
{
    ...
    BACKTRACE();
    SetTx ( 64000 ) ;           // timeout 10s (should not happen, TXDONE irq will be raised)
}
...
static void txfsk (void)
{
    ...
    ClearIrqStatus(IRQ_ALL);
    SetDioIrqParams(IRQ_TXDONE | IRQ_TIMEOUT);
    hal_irqmask_set(HAL_IRQMASK_DIO1) ;                     // enable IRQs in HAL
...
static void rxlora (bool rxcontinuous) {
    ...
    if (!rxcontinuous && LMIC.rxtime - now < 0) {
        ...
        // Arduino cores that does not really support printing without IRQs
        //debug_printf("WARNING: rxtime is %ld ticks in the past! (ramp-up time %ld ms / %ld ticks)\r\n",
        //             now - LMIC.rxtime, osticks2ms(now - t0), now - t0);
        // delay ( 50 ) ;
    }
...
void radio_init ( bool calibrate )
{
    ...
    {
      once = false ;
      rhandle.Init.BaudratePrescaler = 0 ;
      rhandle.ErrorCode = 0 ;
      rhandle.State = HAL_SUBGHZ_STATE_RESET ;
      rstat = HAL_SUBGHZ_Init ( &rhandle ) ;          // Init access to radio module
      ASSERT ( rstat == HAL_OK ) ;
    }
    hal_disableIRQs();
    // reset radio (FSK/STANDBY)
    radio_reset();
    // check reset value of LSB syncword
    ASSERT ( ReadReg(REG_LORASYNCWORDLSB) == 0x24 ) ;
...
// Run by irqjob if a new IRQ status is seen
bool radio_irq_process ( ostime_t irqtime, u1_t diomask )
{
    (void)diomask; // unused

    uint16_t irqflags = GetLastIrqStatus() ;
    debug_printf ( "irq_process, IRQ is 0x%04X\r\n", irqflags ) ;
    // dispatch modem
    if ( isFsk ( LMIC.rps ) )
    {   // FSK modem
        if ( irqflags & IRQ_TXDONE)
        {   // TXDONE
            BACKTRACE() ;
            // save exact tx time
            LMIC.txend = irqtime - FSK_TXDONE_FIXUP ;
        }
        else if ( irqflags & IRQ_RXDONE )
        {   // RXDONE
            BACKTRACE();
            ...
            debug_printf ( "RX[rssi=%d,len=%d]: %h\r\n",
                           LMIC.rssi - RSSI_OFF, LMIC.dataLen,
        ...
        else
        {
            ...
            debug_printf ( "UNEXPECTED RADIO IRQ %04x (after %ld ticks, %.1Fms)\r\n",
                           irqflags, irqtime - LMIC.rxtime,
                           osticks2us ( irqtime - LMIC.rxtime ), 3 ) ;
            TRACE_VAL ( irqflags ) ;
            ASSERT(0) ;
        }
    }
    ...
    SetDioIrqParams ( 0 ) ;                 // mask all IRQs
    ClearIrqStatus ( IRQ_ALL ) ;            // clear IRQ flags
    return true ;                           // radio operation completed
}
